...
async function generateEnvContent(data: SetupData): Promise<string> {
  ...
  env.push(`SERVICES_ENABLED=${data.optionalServices?.nestjs || data.optionalServices?.golang || data.optionalServices?.python ? 'true' : 'false'}`)
  ...
  env.push(`ADMIN_PASSWORD_HASH=${data.adminPassword ? await hashPassword(data.adminPassword) : ''}`)
  env.push('')
