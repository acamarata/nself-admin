...
export async function POST(request: NextRequest) {
  try {
    ...
    let parsedCommand: { command: string; args?: string[]; options?: Record<string, string> }
    ...
    if (typeof body.command === 'string') {
      ...
      for (let i = 2; i < parts.length; i++) {
        if (parts[i].startsWith('--')) {
          ...
          const optValue = (i + 1 < parts.length && !parts[i + 1].startsWith('--')) 
            ? parts[++i] 
            : 'true'
      ...
      if (!ALLOWED_NSELF_COMMANDS[parsedCommand.command]) {
        return NextResponse.json(
          { success: false, error: `Invalid nself command: ${parsedCommand.command}` },
          { status: 403 }
        )
      }
    }
