  ...
  try {
    ...
    if (!result.success) {
      ...
      try {
        ...
        const { stdout } = await execAsync(`ls -la ${backupDir}/*.sql* 2>/dev/null || echo ""`)
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (body.action === 'backup') {
      ...
      const { includeDatabase, includeFiles, includeConfig, compression, encryptionKey } = validation.data
      ...
      const { backupFile, includeDatabase, includeFiles, includeConfig, decryptionKey } = validation.data
      ...
      const result = await executeNselfCommand(
        'backup',
        ['schedule', '--frequency', frequency, '--time', time, '--retention', String(retention)]
      )
