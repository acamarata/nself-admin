...
export async function GET() {
  try {
    ...
    try {
      ...
      const { stdout } = await execAsync(`netstat -ibn | grep -E "^${iface}\\s" | awk '{print $7 " " $10}'`)
      ...
      // Try Linux method
      try {
        const { stdout: rxOut } = await execAsync(`cat /sys/class/net/${iface}/statistics/rx_bytes`)
        const { stdout: txOut } = await execAsync(`cat /sys/class/net/${iface}/statistics/tx_bytes`)
