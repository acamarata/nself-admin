...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      const result = await execAsync(`echo "Y" | ${nselfCommand} build --force`, {
      ...
      const dockerComposePath = path.join(backendProjectPath, 'docker-compose.yml')
      try {
        ...
        return NextResponse.json({
          ...
          output: stdout || `Build completed successfully. ${serviceCount} services configured.`,
          serviceCount
        })
      } catch {
      ...
      try {
        ...
        return NextResponse.json({
          success: true,
          message: 'Project built successfully',
          output: execError.stdout || `Build completed. ${serviceCount} services configured.`,
          serviceCount
        })
      } catch {
        ...
        return NextResponse.json(
          { 
            error: 'Build failed',
            details: execError.stderr || execError.message || 'Unknown error during build',
