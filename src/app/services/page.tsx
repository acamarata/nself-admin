...
const ServiceCard = dynamic(() => import('@/components/services/ServiceCard').then(mod => ({ default: mod.ServiceCard })), {
  loading: () => <div className="animate-pulse bg-zinc-200 dark:bg-zinc-800 rounded-lg h-48" />
})

const ServiceListView = dynamic(() => import('@/components/services/ServiceListView').then(mod => ({ default: mod.ServiceListView })), {
  loading: () => <div className="animate-pulse bg-zinc-200 dark:bg-zinc-800 rounded-lg h-96" />
})
...
function getCategoryColors(category: 'required' | 'optional' | 'user') {
  switch (category) {
    case 'required':
      return {
        badge: 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 border-blue-200 dark:border-blue-800/50',
        dot: 'bg-blue-500',
    ...
    case 'optional':
      return {
        badge: 'bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 border-purple-200 dark:border-purple-800/50',
        dot: 'bg-purple-500',
    ...
    case 'user':
      return {
        badge: 'bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400 border-orange-200 dark:border-orange-800/50',
        dot: 'bg-orange-500',
...
function getServiceCategory(name: string | undefined): 'required' | 'optional' | 'user' {
  ...
  if (['nginx', 'proxy'].some(n => lowerName.includes(n)) && !lowerName.includes('haproxy')) return 'required'
  
  // Optional services (infrastructure)
  if (['minio', 'storage'].some(n => lowerName.includes(n))) return 'optional'
  if (['mailpit', 'mail'].some(n => lowerName.includes(n)) && !lowerName.includes('bullmq')) return 'optional'
...
function getServiceOrder(name: string | undefined): number {
  ...
  if (category === 'optional') {
    ...
    if (lowerName === 'nself_storage' || lowerName === 'nself-storage' || (lowerName.includes('storage') && !lowerName.includes('minio'))) return 201
...
function EnhancedListView({ 
  ...
  const [sortColumn, setSortColumn] = useState<'name' | 'status' | 'cpu' | 'memory' | 'disk' | 'ports' | null>(null)
    ...
    return [...containers].sort((a, b) => {
      ...
      switch (sortColumn) {
        ...
        case 'memory':
          ...
          break
        case 'disk':
          const diskA = a.stats?.disk?.used || (a.stats?.blockIO ? a.stats.blockIO.read + a.stats.blockIO.write : 0)
          const diskB = b.stats?.disk?.used || (b.stats?.blockIO ? b.stats.blockIO.read + b.stats.blockIO.write : 0)
  ...
  function getServiceIcon(name: string | undefined): any {
    ...
    if (lowerName.includes('grafana') || lowerName.includes('prometheus')) return serviceIcons.monitoring
        ...
        <td className="px-4 py-3">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <Icon className={`w-5 h-5 ${categoryColors.icon}`} />
            </div>
            <div>
              <div className="font-medium text-sm text-zinc-900 dark:text-white">{displayName}</div>
        ...
        <td className="px-4 py-3">
          <div>
            ...
            {container.state === 'running' && (
              <div className="text-xs text-zinc-500 mt-0.5">Uptime: {uptime}</div>
            )}
          </div>
        </td>
        ...
        <td className="px-4 py-3 text-center">
          {container.ports && container.ports.length > 0 ? (
            <div className="flex flex-wrap gap-1 justify-center">
              {container.ports.filter(p => p.public).map((port, i) => (
                <span key={i} className="text-xs px-1.5 py-0.5 rounded bg-zinc-100 dark:bg-zinc-800">
                  {port.public}
                </span>
              ))}
            </div>
  ...
  return (
    <div className="rounded-xl overflow-hidden bg-white dark:bg-zinc-900/50 ring-1 ring-zinc-200 dark:ring-zinc-700">
          ...
          <tr>
            <th className="px-4 py-3 text-left">
              <button
                ...
                className="group flex items-center gap-1 text-xs font-medium text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white"
              >
            ...
            <th className="px-4 py-3 text-center">
              <button
                ...
                className="group flex items-center gap-1 text-xs font-medium text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white mx-auto"
              >
            ...
            <>
              <tr>
                <td colSpan={7} className="px-4 py-2 bg-blue-50/50 dark:bg-blue-900/10">
                  <span className="text-xs font-medium text-blue-700 dark:text-blue-400">Required Services</span>
                </td>
              </tr>
            ...
            <>
              <tr>
                <td colSpan={7} className="px-4 py-2 bg-purple-50/50 dark:bg-purple-900/10">
                  <span className="text-xs font-medium text-purple-700 dark:text-purple-400">Optional Services</span>
                </td>
              </tr>
...
function MetricCard({ 
  ...
  return (
    <motion.div
      ...
      className="relative overflow-hidden rounded-3xl bg-zinc-50 dark:bg-zinc-800/50 ring-1 ring-zinc-200 dark:ring-zinc-700 hover:ring-2 hover:ring-blue-500/20 dark:hover:ring-blue-400/20 transition-all"
...
function TreeView({ 
  ...
  const TreeNode = ({ container, indent = 0 }: { container: Container; indent?: number }) => {
    ...
    return (
      <div 
        className="flex items-center gap-3 py-2 px-4 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 rounded-lg"
      ...
      {requiredServices.length > 0 && (
        <div className="border-b border-zinc-200 dark:border-zinc-700">
          <div className="px-4 py-3 bg-blue-50/50 dark:bg-blue-900/10">
            <span className="text-sm font-medium text-blue-700 dark:text-blue-400">Required Services</span>
          </div>
      ...
      {optionalServices.length > 0 && (
        <div className="border-b border-zinc-200 dark:border-zinc-700">
          <div className="px-4 py-3 bg-purple-50/50 dark:bg-purple-900/10">
            <span className="text-sm font-medium text-purple-700 dark:text-purple-400">Optional Services</span>
          </div>
    ...
    if (filter !== 'all') {
      filtered = filtered.filter(c => {
        switch (filter) {
          ...
          case 'stack': return c.category === 'stack'
          case 'services': return c.category === 'services'
          case 'healthy': return c.health === 'healthy'
          case 'unhealthy': return c.health === 'unhealthy'
          default: return true
        }
      })
    }
      ...
      <div className="mb-16">
        <div className="not-prose grid grid-cols-1 gap-8 sm:grid-cols-2 xl:grid-cols-4">
          <MetricCard
            ...
            value={stats.total}
            percentage={typeof stats.running === 'number' && typeof stats.total === 'number' ? (stats.running / stats.total) * 100 : undefined}
          ...
          <MetricCard
            ...
            percentage={typeof stats.healthy === 'number' && typeof stats.total === 'number' ? (stats.healthy / stats.total) * 100 : undefined}
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {sortedContainers.length > 0 ? (
              ...
              <div className="col-span-full text-center py-12">
                <Icons.Server className="h-12 w-12 text-zinc-400 mx-auto mb-4" />
