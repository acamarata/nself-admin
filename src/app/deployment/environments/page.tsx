...
export default function EnvironmentsPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault()
    await executeAction('create', { name: newEnvName, template: newEnvTemplate })
      ...
      <div className="relative mx-auto max-w-7xl">
        ...
        {projectPath && (
          <div className="mb-6 rounded-lg border border-zinc-200 bg-zinc-50 px-4 py-3 dark:border-zinc-700 dark:bg-zinc-800/50">
            <p className="text-sm text-zinc-500 dark:text-zinc-400">
              ...
              <code className="rounded bg-zinc-200 px-1.5 py-0.5 font-mono dark:bg-zinc-700">{projectPath}</code>
            </p>
          </div>
        )}
        ...
        {showCreateForm && (
          <div className="mb-8 rounded-xl border border-blue-200 bg-blue-50 p-6 dark:border-blue-800 dark:bg-blue-900/20">
            <h3 className="mb-4 text-lg font-semibold text-zinc-900 dark:text-white">Create New Environment</h3>
            <form onSubmit={handleCreate} className="space-y-4">
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  <label className="mb-1 block text-sm font-medium text-zinc-700 dark:text-zinc-300">Environment Name *</label>
                  <input
                    type="text"
                    value={newEnvName}
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setNewEnvTemplate(e.target.value as 'local' | 'staging' | 'prod')}
                    className="w-full rounded-lg border border-zinc-300 bg-white px-4 py-2 text-zinc-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-zinc-600 dark:bg-zinc-800 dark:text-white"
                  >
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                {(actionResult.output || actionResult.error) && (
                  <pre className="mt-2 max-h-40 overflow-auto whitespace-pre-wrap rounded-lg bg-zinc-900 p-4 font-mono text-sm text-zinc-100">
                    {actionResult.output || actionResult.error}
                  </pre>
                )}
              </div>
        ...
        {environments.length === 0 ? (
          <div className="rounded-xl border border-zinc-200 bg-white p-12 text-center shadow-sm dark:border-zinc-700 dark:bg-zinc-800">
            ...
            <h3 className="mb-2 text-lg font-semibold text-zinc-900 dark:text-white">No Environments Found</h3>
              ...
              >
                ...
                <div className="mb-4 flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <div className={`flex h-10 w-10 items-center justify-center rounded-lg ${getTypeColor(env.type)}`}>
                      {getTypeIcon(env.type)}
                    </div>
                    <div>
                      <h3 className="font-semibold text-zinc-900 dark:text-white">{env.name}</h3>
                      <span className={`inline-block rounded px-2 py-0.5 text-xs font-medium capitalize ${getTypeColor(env.type)}`}>
                        {env.type}
                      </span>
                    </div>
                  </div>
                ...
                <div className="mb-4 space-y-2">
                  <div className="flex items-center gap-2 text-sm">
                    <div className={`h-2 w-2 rounded-full ${env.hasEnv ? 'bg-green-500' : 'bg-red-500'}`} />
                    <span className="text-zinc-600 dark:text-zinc-400">.env file</span>
                  </div>
                  <div className="flex items-center gap-2 text-sm">
                    <div className={`h-2 w-2 rounded-full ${env.hasSecrets ? 'bg-green-500' : 'bg-yellow-500'}`} />
                    <span className="text-zinc-600 dark:text-zinc-400">.env.secrets</span>
                  </div>
                  <div className="flex items-center gap-2 text-sm">
                    <div className={`h-2 w-2 rounded-full ${env.hasServer ? 'bg-green-500' : 'bg-zinc-300'}`} />
                ...
                {env.serverConfig?.host && (
                  <div className="mb-4 rounded-lg bg-zinc-50 p-3 dark:bg-zinc-900">
                    ...
                    {env.serverConfig.deployPath && (
                      <p className="mt-1 font-mono text-xs text-zinc-500">{env.serverConfig.deployPath}</p>
                    )}
                  </div>
                )}
        ...
        <div className="mt-8 rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-700 dark:bg-zinc-800">
          ...
          <div className="space-y-2 font-mono text-sm">
            ...
            <p className="text-zinc-600 dark:text-zinc-400">
              <span className="text-blue-500">nself env create</span> &lt;name&gt; [template] - Create environment
            </p>
            <p className="text-zinc-600 dark:text-zinc-400">
              <span className="text-blue-500">nself env switch</span> &lt;name&gt; - Switch to environment
            </p>
            <p className="text-zinc-600 dark:text-zinc-400">
              <span className="text-blue-500">nself env status</span> - Show current environment status
            </p>
            <p className="text-zinc-600 dark:text-zinc-400">
              <span className="text-blue-500">nself env diff</span> &lt;env1&gt; &lt;env2&gt; - Compare environments
            </p>
            <p className="text-zinc-600 dark:text-zinc-400">
              <span className="text-blue-500">nself env validate</span> [name] - Validate environment config
            </p>
          </div>
        </div>
      </div>
