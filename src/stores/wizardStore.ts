...
export const useWizardStore = create<WizardState>()((set, get) => ({
      ...
      setBasicConfig: (config) => set((state) => ({
        ...state,
        ...config,
        lastSync: Date.now()
      })),
      
      setServiceConfig: (service, config) => set((state) => ({
        ...state,
        [`${service}Config`]: config,
        lastSync: Date.now()
      })),
      
      setOptionalServices: (services) => set((state) => ({
        ...state,
        optionalServices: services,
        lastSync: Date.now()
      })),
      
      setCustomServices: (services) => set((state) => ({
        ...state,
        customServices: services,
        lastSync: Date.now()
      })),
      
      setFrontendApps: (apps) => set((state) => ({
        ...state,
        frontendApps: apps,
        lastSync: Date.now()
      })),
      ...
      loadFromEnv: (envData) => {
        ...
        if (envData.HASURA_GRAPHQL_ADMIN_SECRET) state.hasuraAdminSecret = envData.HASURA_GRAPHQL_ADMIN_SECRET
