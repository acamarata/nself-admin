...
import { loadEnvironmentVariables, isDevelopment } from './env-loader'
...
const WEAK_PASSWORDS = [
  'admin123', 'password', 'changeme', 'changeme123', 'default',
  'admin', 'test', 'test123', 'demo', 'demo123', 'password123',
  '123456', '12345678', 'qwerty', 'letmein', 'welcome',
  'monkey', 'dragon', 'master', 'admin1234', 'pass1234'
]
...
export async function hashPassword(password: string): Promise<string> {
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (password.length >= 4) {
      console.warn('⚠️ Weak password accepted in development mode. Use a strong password in production!')
