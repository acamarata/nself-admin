...
interface LogEntry {
  id: string
  timestamp: string
  type: 'click' | 'navigation' | 'api' | 'error' | 'performance' | 'state' | 'render' | 'network' | 'user'
...
class DevLogger {
  ...
  private interceptConsole() {
    ...
    methods.forEach(method => {
      ...
      console[method as keyof Console] = (...args: any[]) => {
        ...
        this.log(level, 'console', args.map(a => 
  ...
  getStats() {
    ...
    if (apiLogs.length > 0) {
      stats.avgApiDuration = apiLogs.reduce((sum, log) => sum + (log.data?.duration || 0), 0) / apiLogs.length
    }
