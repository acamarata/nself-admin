...
- [ ] **Variable list**
  ...
  - Group by prefix (DB_, HASURA_, etc.)
- [ ] **Add variable**
...
test('view services and start/stop', async ({ page }) => {
  ...
  await expect(page.locator('[data-service="postgres"] [data-status]')).toHaveAttribute('data-status', 'stopped')

  // Start again
  await page.click('[data-service="postgres"] button:has-text("Start")')
  await expect(page.locator('[data-service="postgres"] [data-status]')).toHaveAttribute('data-status', 'running')
})
```
